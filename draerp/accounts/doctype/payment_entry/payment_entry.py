...
import draerp
from draerp.accounts.doctype.bank_account.bank_account import (
	get_bank_account_details,
	get_party_bank_account,
)
from draerp.accounts.doctype.invoice_discounting.invoice_discounting import (
	get_party_account_based_on_invoice_discounting,
)
from draerp.accounts.doctype.journal_entry.journal_entry import get_default_bank_cash_account
from draerp.accounts.doctype.tax_withholding_category.tax_withholding_category import (
	get_party_tax_withholding_details,
)
from draerp.accounts.general_ledger import make_gl_entries, process_gl_map
from draerp.accounts.party import get_party_account
from draerp.accounts.utils import get_account_currency, get_balance_on, get_outstanding_invoices
from draerp.controllers.accounts_controller import (
	AccountsController,
...
class PaymentEntry(AccountsController):
	...
	def set_payment_req_status(self):
		from draerp.accounts.doctype.payment_request.payment_request import update_payment_req_status
		...
		if self.party:
			...
			if self.party_account and self.party_type in ("Customer", "Supplier"):
				self.validate_account_type(
					self.party_account, [draerp.get_party_account_type(self.party_type)]
				)
	...
	def add_party_gl_entries(self, gl_entries):
		if self.party_account:
			...
			dr_or_cr = (
				"credit" if draerp.get_party_account_type(self.party_type) == "Receivable" else "debit"
			)
...
def get_reference_details(reference_doctype, reference_name, party_account_currency):
	...
	company_currency = ref_doc.get("company_currency") or draerp.get_company_currency(
		ref_doc.company
	)
