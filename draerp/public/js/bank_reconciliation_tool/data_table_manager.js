...
draerp.accounts.bank_reconciliation.DataTableManager = class DataTableManager {
	constructor(opts) {
		...
		this.dialog_manager = new draerp.accounts.bank_reconciliation.DialogManager(
			this.company,
			this.bank_account
		);
		this.make_dt();
	...
	make_dt() {
		...
		frappe.call({
			method:
				"draerp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.get_bank_transactions",
			args: {
