...
frappe.setup.on("before_load", function () {
	draerp.setup.slides_settings.map(frappe.setup.add_slide);
});

draerp.setup.slides_settings = [
	{
		// Brand
		name: 'brand',
		icon: "fa fa-bookmark",
	...
	},
	{
		...
		validate: function () {
			...
			if(me.values.bank_account) {
				frappe.call({
					async: false,
					method: "draerp.accounts.doctype.account.chart_of_accounts.chart_of_accounts.validate_bank_account",
					args: {
		...
		set_fy_dates: function (slide) {
			...
			if (country) {
				var fy = draerp.setup.fiscal_years[country];
		...
		charts_modal: function(slide, chart_template) {
			...
			let coa_tree = new frappe.ui.Tree({
				...
				label: parent,
				expandable: true,
				method: 'draerp.accounts.utils.get_coa',
				args: {
