...
draerp.TransactionController = class TransactionController extends draerp.taxes_and_totals {
							...
							frappe.run_serially([
								...
								() => {
									if(show_batch_dialog && !frappe.flags.hide_serial_batch_dialog) {
										...
										draerp.show_serial_batch_selector(me.frm, d, (item) => {
	...
	apply_price_list(item, reset_plc_conversion) {
		...
		// `draerp.stock.get_item_details.apply_price_list` is sensitive to its value
	...
	setup_item_selector() {
		...
		// 	this.item_selector = new draerp.ItemSelector({frm: this.frm});
		// }
	}
