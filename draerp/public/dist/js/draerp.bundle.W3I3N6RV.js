(() => {
  // ../draerp/draerp/public/js/conf.js
  frappe.provide("draerp");
  ...
  $.extend(frappe.breadcrumbs.module_map, {
    "DraERP Integrations": "Integrations",
    "Geo": "Settings",
    "Portal": "Website",
    "Utilities": "Settings",
    "E-commerce": "Website",
  ...
  // ../draerp/draerp/public/js/utils.js
  frappe.provide("draerp");
  frappe.provide("draerp.utils");
  $.extend(draerp, {
    ...
    hide_company: function() {
      if (cur_frm.fields_dict.company) {
        ...
        } else if (draerp.last_selected_company) {
          if (!cur_frm.doc.company)
            cur_frm.set_value("company", draerp.last_selected_company);
        }
      }
    },
  ...
  $.extend(draerp.utils, {
      ...
      if (frm.doc.__onload && frm.doc.__onload.dashboard_info) {
        ...
        if (company_wise_info.length > 1) {
          company_wise_info.forEach(function(info) {
            draerp.utils.add_indicator_for_multicompany(frm, info);
          });
    ...
    make_bank_account: function(doctype, docname) {
      frappe.call({
        method: "draerp.accounts.doctype.bank_account.bank_account.make_bank_account",
        args: {
          doctype,
          docname
        },
    ...
    add_dimensions: function(report_name, index) {
      ...
      frappe.call({
        method: "draerp.accounts.doctype.accounting_dimension.accounting_dimension.get_dimensions",
        callback: function(r) {
    ...
    add_inventory_dimensions: function(report_name, index) {
      let filters = frappe.query_reports[report_name].filters;
      frappe.call({
        method: "draerp.stock.doctype.inventory_dimension.inventory_dimension.get_inventory_dimensions",
        callback: function(r) {
    ...
    make_pricing_rule: function(doctype, docname) {
      frappe.call({
        method: "draerp.accounts.doctype.pricing_rule.pricing_rule.make_pricing_rule",
        args: {
          doctype,
          docname
        },
  ...
  draerp.utils.select_alternate_items = function(opts) {
    const frm = opts.frm;
          ...
          fields: [{
            ...
            onchange: function() {
              ...
              if (item_code && warehouse) {
                frappe.call({
                  method: "draerp.stock.utils.get_latest_stock_qty",
                  args: {
                    item_code,
                    warehouse
                  },
            ...
            get_query: (e) => {
              return {
                query: "draerp.stock.doctype.item_alternative.item_alternative.get_alternative_items",
                filters: {
  ...
  draerp.utils.update_child_items = function(opts) {
    const frm = opts.frm;
    ...
    const fields = [{
      ...
      get_query: function() {
        ...
        return {
          query: "draerp.controllers.queries.item_query",
          filters
        };
      }
    }, {
      ...
      onchange: function() {
        frappe.call({
          method: "draerp.stock.get_item_details.get_conversion_factor",
    ...
    const dialog = new frappe.ui.Dialog({
      ...
      primary_action: function() {
        ...
        frappe.call({
          method: "draerp.controllers.accounts_controller.update_child_qty_rate",
          freeze: true,
          args: {
  ...
  draerp.utils.map_current_doc = function(opts) {
    function _map() {
  ...
  $(document).on("app_ready", function() {
    frappe.call({
      method: "draerp.support.doctype.service_level_agreement.service_level_agreement.get_sla_doctypes",
      callback: function(r) {
        if (!r.message)
          return;
          ...
          frappe.ui.form.on(d, {
            onload: function(frm) {
              if (!frm.doc.service_level_agreement)
                return;
              frappe.call({
                method: "draerp.support.doctype.service_level_agreement.service_level_agreement.get_service_level_agreement_filters",
                args: {
  ...
  // ../draerp/draerp/public/js/queries.js
  frappe.provide("draerp.queries");
  $.extend(draerp.queries, {
    user: function() {
      return { query: "frappe.core.doctype.user.user.user_query" };
    },
    lead: function() {
      return { query: "draerp.controllers.queries.lead_query" };
    },
    customer: function() {
      return { query: "draerp.controllers.queries.customer_query" };
    },
    supplier: function() {
      return { query: "draerp.controllers.queries.supplier_query" };
    },
    item: function(filters) {
      var args = { query: "draerp.controllers.queries.item_query" };
      if (filters)
        args["filters"] = filters;
      return args;
    },
    bom: function() {
      return { query: "draerp.controllers.queries.bom" };
    },
    task: function() {
      return { query: "draerp.projects.utils.query_task" };
    },
    ...
    employee: function() {
      return { query: "draerp.controllers.queries.employee_query" };
    },
    ...
    get_filtered_dimensions: function(doc, child_fields, dimension, company) {
      ...
      return {
        query: "draerp.controllers.queries.get_filtered_dimensions",
        filters: {
          "dimension": dimension,
          "account": account,
          "company": company
  ...
  draerp.queries.setup_queries = function(frm, options, query_fn) {
    var me = this;
  ...
  draerp.queries.setup_warehouse_query = function(frm) {
    frm.set_query("warehouse", "items", function(doc, cdt, cdn) {
      var row = locals[cdt][cdn];
      var filters = draerp.queries.warehouse(frm.doc);
      if (row.item_code) {
        $.extend(filters, { "query": "draerp.controllers.queries.warehouse_query" });
  ...
  // ../draerp/draerp/public/js/sms_manager.js
  draerp.SMSManager = function SMSManager(doc) {
    var me = this;
    this.setup = function() {
      var default_msg = {
        "Lead": "",
  ...
  // ../draerp/draerp/public/js/utils/party.js
  frappe.provide("draerp.utils");
  ...
  draerp.utils.get_party_details = function(frm, method, args, callback) {
    if (!method) {
      method = "draerp.accounts.party.get_party_details";
    }
    if (!args) {
    ...
    if (frappe.meta.get_docfield(frm.doc.doctype, "taxes")) {
      if (!draerp.utils.validate_mandatory(frm, "Posting / Transaction Date", args.posting_date, args.party_type == "Customer" ? "customer" : "supplier"))
        return;
    }
    if (!draerp.utils.validate_mandatory(frm, "Company", frm.doc.company, args.party_type == "Customer" ? "customer" : "supplier")) {
      return;
    }
  ...
  draerp.utils.add_item = function(frm) {
    if (frm.is_new()) {
  ...
  draerp.utils.get_address_display = function(frm, address_field, display_field, is_your_company_address) {
  ...
  draerp.utils.set_taxes_from_address = function(frm, triggered_from_field, billing_address_field, shipping_address_field) {
    if (frm.updating_party_details)
      return;
    if (frappe.meta.get_docfield(frm.doc.doctype, "taxes")) {
      if (!draerp.utils.validate_mandatory(frm, "Lead / Customer / Supplier", frm.doc.customer || frm.doc.supplier || frm.doc.lead || frm.doc.party_name, triggered_from_field)) {
        return;
      }
      if (!draerp.utils.validate_mandatory(frm, "Posting / Transaction Date", frm.doc.posting_date || frm.doc.transaction_date, triggered_from_field)) {
        return;
      }
    } else {
      return;
    ...
    frappe.call({
      method: "draerp.accounts.party.get_address_tax_category",
      args: {
      ...
      callback: function(r) {
        if (!r.exc) {
          if (frm.doc.tax_category != r.message) {
            ...
            draerp.utils.set_taxes(frm, triggered_from_field);
          }
        }
      }
    });
  };
  draerp.utils.set_taxes = function(frm, triggered_from_field) {
    if (frappe.meta.get_docfield(frm.doc.doctype, "taxes")) {
      if (!draerp.utils.validate_mandatory(frm, "Company", frm.doc.company, triggered_from_field)) {
        return;
      }
      if (!draerp.utils.validate_mandatory(frm, "Lead / Customer / Supplier", frm.doc.customer || frm.doc.supplier || frm.doc.lead || frm.doc.party_name, triggered_from_field)) {
        return;
    ...
    frappe.call({
      method: "draerp.accounts.party.set_taxes",
      args: {
        "party": party,
  ...
  draerp.utils.get_contact_details = function(frm) {
    if (frm.updating_party_details)
      return;
  ...
  draerp.utils.validate_mandatory = function(frm, label, value, trigger_on) {
    if (!value) {
      frm.doc[trigger_on] = "";
  ...
  draerp.utils.get_shipping_address = function(frm, callback) {
    if (frm.doc.company) {
      ...
      frappe.call({
        method: "draerp.accounts.custom.address.get_shipping_address",
        args: {
          company: frm.doc.company,
  ...
  // ../draerp/draerp/public/js/controllers/stock_controller.js
  frappe.provide("draerp.stock");
  draerp.stock.StockController = class StockController extends frappe.ui.form.Controller {
    onload() {
    ...
    setup_warehouse_query() {
      var me = this;
      draerp.queries.setup_queries(this.frm, "Warehouse", function() {
        return draerp.queries.warehouse(me.frm.doc);
      });
    }
  ...
  // ../draerp/draerp/public/js/payment/payments.js
  draerp.payments = class payments extends draerp.stock.StockController {
    make_payment() {
      var me = this;
  ...
  // ../draerp/draerp/public/js/controllers/taxes_and_totals.js
  draerp.taxes_and_totals = class TaxesAndTotals extends draerp.payments {
    setup() {
    ...
    fetch_round_off_accounts() {
      ...
      if (me.frm.doc.company) {
        return frappe.call({
          "method": "draerp.controllers.taxes_and_totals.get_round_off_applicable_accounts",
          "args": {
            "company": me.frm.doc.company,
  ...
  // ../draerp/draerp/public/js/controllers/transaction.js
  draerp.TransactionController = class TransactionController extends draerp.taxes_and_totals {
    setup() {
      super.setup();
      let me = this;
      ...
      frappe.ui.form.on(this.frm.doctype + " Item", {
        items_add: function(frm, cdt, cdn) {
          ...
          draerp.accounts.dimensions.copy_dimension_from_first_row(frm, cdt, cdn, "items");
        }
      });
      ...
      if (this.frm.fields_dict["items"].grid.get_field("blanket_order")) {
        this.frm.set_query("blanket_order", "items", function(doc, cdt, cdn) {
          ...
          return {
            query: "draerp.controllers.queries.get_blanket_orders",
            filters: {
              "company": doc.company,
    ...
    scan_barcode() {
      const barcode_scanner = new draerp.utils.BarcodeScanner({ frm: this.frm });
    ...
    apply_default_taxes() {
      ...
      if (taxes_and_charges_field) {
        return frappe.call({
          method: "draerp.controllers.accounts_controller.get_default_taxes_and_charges",
          args: {
    ...
    send_sms() {
      var sms_man = new draerp.SMSManager(this.frm.doc);
    }
      ...
      if (item.item_code || item.serial_no) {
        if (!this.validate_company_and_party()) {
          ...
          return this.frm.call({
            method: "draerp.stock.get_item_details.get_item_details",
            child: item,
            args: {
              doc: me.frm.doc,
              args: {
                ...
                frappe.run_serially([
                  ...
                  () => {
                    if (show_batch_dialog && !frappe.flags.hide_serial_batch_dialog) {
                      ...
                      draerp.show_serial_batch_selector(me.frm, d, (item2) => {
    ...
    get_incoming_rate(item, posting_date, posting_time, voucher_type, company) {
      ...
      frappe.call({
        method: "draerp.stock.utils.get_incoming_rate",
        args: {
          args: item_args
        },
        callback: function(r) {
    ...
    company() {
      ...
      var set_party_account = function(set_pricing2) {
        if (in_list(["Sales Invoice", "Purchase Invoice"], me.frm.doc.doctype)) {
          ...
          if (party && me.frm.doc.company) {
            return frappe.call({
              method: "draerp.accounts.party.get_party_account",
              args: {
                company: me.frm.doc.company,
                party_type,
                party
      ...
      if (frappe.meta.get_docfield(this.frm.doctype, "shipping_address") && in_list(["Purchase Order", "Purchase Receipt", "Purchase Invoice"], this.frm.doctype)) {
        draerp.utils.get_shipping_address(this.frm, function() {
      ...
      if (this.frm.doc.company) {
        draerp.last_selected_company = this.frm.doc.company;
      }
    }
    transaction_date() {
    ...
    posting_date() {
      ...
      if (this.frm.doc.posting_date) {
        ...
        if (this.frm.doc.doctype == "Sales Invoice" && this.frm.doc.customer || this.frm.doc.doctype == "Purchase Invoice" && this.frm.doc.supplier) {
          return frappe.call({
            method: "draerp.accounts.party.get_due_date",
            args: {
    ...
    get_company_currency() {
      return draerp.get_currency(this.frm.doc.company);
    }
    contact_person() {
      draerp.utils.get_contact_details(this.frm);
    }
    currency() {
    ...
    conversion_rate() {
      ...
      this.frm.set_df_property("conversion_rate", "read_only", draerp.stale_rate_allowed() ? 0 : 1);
    }
    shipping_rule() {
      var me = this;
    ...
    get_exchange_rate(transaction_date, from_currency, to_currency, callback) {
      ...
      if (!transaction_date || !from_currency || !to_currency)
        return;
      return frappe.call({
        method: "draerp.setup.utils.get_exchange_rate",
        args: {
          transaction_date,
          from_currency,
          to_currency,
    ...
    service_start_date(frm, cdt, cdn) {
      ...
      if (child.service_start_date) {
        frappe.call({
          "method": "draerp.stock.get_item_details.calculate_service_end_date",
          args: { "args": child },
          callback: function(r) {
    ...
    ignore_pricing_rule() {
      if (this.frm.doc.ignore_pricing_rule) {
        ...
        return this.frm.call({
          method: "draerp.accounts.doctype.pricing_rule.pricing_rule.remove_pricing_rules",
          args: { item_list },
          callback: function(r) {
            if (!r.exc && r.message) {
    ...
    apply_pricing_rule(item, calculate_taxes_and_totals) {
      ...
      return this.frm.call({
        method: "draerp.accounts.doctype.pricing_rule.pricing_rule.apply_pricing_rule",
    ...
    apply_price_list(item, reset_plc_conversion) {
      ...
      return this.frm.call({
        method: "draerp.stock.get_item_details.apply_price_list",
        args: { args },
        callback: function(r) {
          if (!r.exc) {
            frappe.run_serially([
    ...
    get_terms() {
      var me = this;
      draerp.utils.get_terms(this.frm.doc.tc_name, this.frm.doc, function(r) {
        if (!r.exc) {
    ...
    taxes_and_charges() {
      ...
      if (this.frm.doc.taxes_and_charges) {
        return this.frm.call({
          method: "draerp.controllers.accounts_controller.get_taxes_and_charges",
          args: {
    ...
    tax_category() {
      ...
      frappe.run_serially([
        ...
        () => draerp.utils.set_taxes(this.frm, "tax_category")
      ]);
    }
    update_item_tax_map() {
      let me = this;
      ...
      if (item_codes.length) {
        return this.frm.call({
          method: "draerp.stock.get_item_details.get_item_tax_info",
          args: {
            company: me.frm.doc.company,
    ...
    item_tax_template(doc, cdt, cdn) {
      ...
      if (item.item_tax_template) {
        return this.frm.call({
          method: "draerp.stock.get_item_details.get_item_tax_map",
          args: {
            company: me.frm.doc.company,
            item_tax_template: item.item_tax_template,
            as_json: true
    ...
    make_quality_inspection() {
      ...
      const dialog = new frappe.ui.Dialog({
        ...
        primary_action: function() {
          ...
          frappe.call({
            method: "draerp.controllers.stock_controller.make_quality_inspections",
            args: {
              doctype: me.frm.doc.doctype,
    ...
    get_method_for_payment() {
      var method = "draerp.accounts.doctype.payment_entry.payment_entry.get_payment_entry";
      if (cur_frm.doc.__onload && cur_frm.doc.__onload.make_payment_via_journal_entry) {
        if (in_list(["Sales Invoice", "Purchase Invoice"], cur_frm.doc.doctype)) {
          method = "draerp.accounts.doctype.journal_entry.journal_entry.get_payment_entry_against_invoice";
        } else {
          method = "draerp.accounts.doctype.journal_entry.journal_entry.get_payment_entry_against_order";
        }
      }
      return method;
    }
    set_query_for_batch(doc, cdt, cdn) {
      ...
      if (!item.item_code) {
        ...
        return {
          query: "draerp.controllers.queries.get_batch_no",
          filters
        };
      }
    }
    set_query_for_item_tax_template(doc, cdt, cdn) {
      ...
      if (!item.item_code) {
        ...
        return {
          query: "draerp.controllers.queries.get_tax_template",
          filters
        };
      }
    }
    payment_terms_template() {
      ...
      if (doc.payment_terms_template && doc.doctype !== "Delivery Note") {
        ...
        frappe.call({
          method: "draerp.controllers.accounts_controller.get_payment_terms",
          args: {
    ...
    payment_term(doc, cdt, cdn) {
      ...
      if (row.payment_term) {
        frappe.call({
          method: "draerp.controllers.accounts_controller.get_payment_term_details",
          args: {
    ...
    blanket_order(doc, cdt, cdn) {
      ...
      if (item.blanket_order && (item.parenttype == "Sales Order" || item.parenttype == "Purchase Order")) {
        frappe.call({
          method: "draerp.stock.get_item_details.get_blanket_order_details",
          args: {
            args: {
              item_code: item.item_code,
  ...
  draerp.show_serial_batch_selector = function(frm, d, callback, on_close, show_dialog) {
    ...
    frappe.require("assets/draerp/js/utils/serial_no_batch_selector.js", function() {
      new draerp.SerialNoBatchSelector({
        frm,
        item: d,
        warehouse_details: {
          type: "Warehouse",
  ...
  draerp.apply_putaway_rule = (frm, purpose = null) => {
    if (!frm.doc.company) {
    ...
    if (!frm.doc.items.length)
      return;
    frappe.call({
      method: "draerp.stock.doctype.putaway_rule.putaway_rule.apply_putaway_rule",
      args: {
        doctype: frm.doctype,
        items: frm.doc.items,
        company: frm.doc.company,
  ...
  // frappe-html:/home/bench/draerp/apps/draerp/draerp/public/js/templates/item_selector.html
...
`;
  ...
  // ../draerp/draerp/public/js/utils/item_selector.js
  draerp.ItemSelector = class ItemSelector {
    constructor(opts) {
      $.extend(this, opts);
      if (!this.item_field) {
      ...
      if (!this.item_query) {
        this.item_query = draerp.queries.item().query;
      }
  ...
  // ../draerp/draerp/public/js/help_links.js
  frappe.provide("frappe.help.help_links");
  var docsUrl = "https://draerp.com/docs/";
  frappe.help.help_links["Form/Rename Tool"] = [
    {
      label: "Bulk Rename",
      url: docsUrl + "user/manual/en/using-draerp/articles/bulk-rename"
    }
  ];
  ...
  frappe.help.help_links["Form/PayPal Settings"] = [
    {
      ...
      url: docsUrl + "user/manual/en/draerp_integration/paypal-integration"
    }
  ];
  frappe.help.help_links["Form/Razorpay Settings"] = [
    {
      label: "Razorpay Settings",
      url: docsUrl + "user/manual/en/draerp_integration/razorpay-integration"
    }
  ];
  frappe.help.help_links["Form/Dropbox Settings"] = [
    {
      label: "Dropbox Settings",
      url: docsUrl + "user/manual/en/draerp_integration/dropbox-backup"
    }
  ];
  frappe.help.help_links["Form/LDAP Settings"] = [
    {
      label: "LDAP Settings",
      url: docsUrl + "user/manual/en/draerp_integration/ldap-integration"
    }
  ];
  frappe.help.help_links["Form/Stripe Settings"] = [
    {
      label: "Stripe Settings",
      url: docsUrl + "user/manual/en/draerp_integration/stripe-integration"
    }
  ];
  ...
  frappe.help.help_links["List/Authorization Rule"] = [
    {
      ...
      url: docsUrl + "user/manual/en/customize-draerp/authorization-rule"
    }
  ];
  ...
  frappe.help.help_links["List/Item"] = [
    ...
    },
    {
      ...
      url: docsUrl + "user/manual/en/stock/articles/calculation-of-valuation-rate-in-fifo-and-moving-average"
    }
  ];
  ...
  frappe.help.help_links["Form/Customize Form"] = [
    {
      label: "Custom Field",
      url: docsUrl + "user/manual/en/customize-draerp/custom-field"
    },
    {
      label: "Customize Field",
      url: docsUrl + "user/manual/en/customize-draerp/customize-form"
    }
  ];
  ...
  // ../draerp/draerp/public/js/agriculture/ternary_plot.js
  ...
  // frappe-html:/home/bench/draerp/apps/draerp/draerp/public/js/templates/item_quick_entry.html
...
`;
  ...
  // ../draerp/draerp/public/js/utils/item_quick_entry.js
  ...
  frappe.ui.form.ItemQuickEntryForm = class ItemQuickEntryForm extends frappe.ui.form.QuickEntryForm {
    ...
    get_variant_doc() {
      ...
      if (Object.keys(attribute).length) {
        frappe.call({
          method: "draerp.controllers.item_variant.create_variant_doc_for_quick_entry",
          args: {
  ...
  // ../draerp/draerp/public/js/utils/customer_quick_entry.js
  frappe.provide("frappe.ui.form");
  ...
  // ../draerp/draerp/public/js/utils/supplier_quick_entry.js
  frappe.provide("frappe.ui.form");
  ...
  // ../draerp/draerp/public/js/call_popup/call_popup.js
  var CallPopup = class {
      ...
      this.call_details = new frappe.ui.FieldGroup({
        fields: [{
          ...
          "click": () => {
            ...
            if (!call_summary)
              return;
            frappe.xcall("draerp.telephony.doctype.call_log.call_log.add_call_summary_and_call_type", {
  ...
  $(document).on("app_ready", function() {
    frappe.realtime.on("show_call_popup", (call_log) => {
      let call_popup = draerp.call_popup;
      if (call_popup && call_log.name === call_popup.call_log.name) {
        call_popup.update_call_log(call_log);
        call_popup.dialog.show();
      } else {
        draerp.call_popup = new CallPopup(call_log);
      }
    });
  });
  window.CallPopup = CallPopup;

  // ../draerp/draerp/public/js/utils/dimension_tree_filter.js
  frappe.provide("draerp.accounts");
  draerp.accounts.dimensions = {
      ...
      if (frappe.meta.has_field(doctype, dimension)) {
        frappe.model.with_doctype(doctype, () => {
          ...
          frm.set_query(dimension, parentfield, function(doc, cdt, cdn) {
            ...
            return draerp.queries.get_filtered_dimensions(row, fields, dimension, doc.company);
          });
        });
      }
    },
    setup_account_filters(frm, dimension, fields) {
      frm.set_query(dimension, function(doc) {
        return draerp.queries.get_filtered_dimensions(doc, fields, dimension, doc.company);
      });
    },
  ...
  // ../draerp/draerp/public/js/utils/barcode_scanner.js
  draerp.utils.BarcodeScanner = class BarcodeScanner {
    constructor(opts) {
      this.frm = opts.frm;
      ...
      this.scan_api = opts.scan_api || "draerp.stock.utils.scan_barcode";
    }
    process_scan() {
    ...
    get_row_to_modify_on_scan(item_code, batch_no, uom) {
      ...
      const matching_row = (row) => {
        ...
        return item_match && uom_match && (!is_batch_no_scan || batch_match) && (!check_max_qty || qty_in_limit);
      };
  ...
  // ../draerp/draerp/public/js/telephony.js
  ...
  // frappe-html:/home/bench/draerp/apps/draerp/draerp/public/js/templates/call_link.html
...
`;
  ...
  // ../draerp/draerp/public/js/bulk_transaction_processing.js
  frappe.provide("draerp.bulk_transaction_processing");
  $.extend(draerp.bulk_transaction_processing, {
    create: function(listview, from_doctype, to_doctype) {
      ...
      frappe.confirm(__("Create {0} {1} ?", [count_of_rows, to_doctype]), () => {
        if (doc_name.length == 0) {
          frappe.call({
            method: "draerp.utilities.bulk_transaction.transaction_processing",
  ...
  // ../draerp/draerp/public/js/utils/crm_activities.js
  draerp.utils.CRMActivities = class CRMActivities {
    constructor(opts) {
      $.extend(this, opts);
    }
    refresh() {
      ...
      frappe.call({
        method: "draerp.crm.utils.get_open_activities",
        args: {
  ...
  draerp.utils.CRMNotes = class CRMNotes {
    constructor(opts) {
      $.extend(this, opts);
    }
    refresh() {
  ...
  // frappe-html:/home/bench/draerp/apps/draerp/draerp/public/js/templates/crm_activities.html
...
.form-footer {
  ...
  // frappe-html:/home/bench/draerp/apps/draerp/draerp/public/js/templates/crm_notes.html
...
//# sourceMappingURL=draerp.bundle.W3I3N6RV.js.map
